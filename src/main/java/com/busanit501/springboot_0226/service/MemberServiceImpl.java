...
import com.busanit501.springboot_0226.dto.MemberJoinDTO;
import com.busanit501.springboot_0226.repository.MemberRepository;
...
public class MemberServiceImpl implements MemberService {
    ...
    public void join(MemberJoinDTO memberJoinDTO) throws MidExistException {
        ...
        // 2. 회원 가입 가능하면, 진행.
        SpringMember springMember = modelMapper.map(memberJoinDTO, SpringMember.class);
        // 화면에서 전달받은 패스워드는 일반 평문 패스워드 -> 암호화 해주기.
        springMember.changePassword(passwordEncoder.encode(memberJoinDTO.getMpw()));
        // 권한 -> 일반 회원 , 관리자,
        springMember.addRole(MemberRole.USER);
        //
        log.info("멤버 서비스에서, 회원 가입 로직 진행중이고, 변환 된 데이터 확인 member: " + springMember);
