...
public interface MemberRepository extends JpaRepository<SpringMember,String> {
    ...
    @Query("select m from SpringMember m where m.mid = :mid and m.social = false")
    Optional<SpringMember> getWithRoles(String mid);

    // 추가로, 소셜 로그인시, 유저 정보를 확인.
    @EntityGraph(attributePaths = "roleSet")
    Optional<SpringMember> findByEmail(String email);

    // 기능추가, 유저의 패스워드를 변경하는 기능.
    @Modifying
    @Transactional
    @Query("update SpringMember m set m.mpw = :mpw where m.mid = :mid")
